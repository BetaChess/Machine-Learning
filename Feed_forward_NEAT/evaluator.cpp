...
void Evaluator::evaluate_training()
{
	...
	for (auto& s : species_)
	{
		...
		{
			nextGenGenomes.emplace_back(*s.memberGenomes[0]);
		}
	}
	
	// Breed the remaining genomes (25% of the next generation should be offspring resulting from mutation without crossover).
